...
::       %1 => The value of the job's ID column if it has one, `null` if no ID value.
::       %2 => The row index in the spreadsheet.  This is always `null` when Bot is enabled.
...
::       %7 => Absolute path to a .json file containing all job's column values
::
::   Provided for your personal or commercial use by Dataclay, LLC

@ECHO OFF

SET log=%6\post-job.log
For /f "tokens=2-4 delims=/ " %%a in ('date /t') do (set mydate=%%c-%%a-%%b)
For /f "tokens=1-2 delims=/:" %%a in ('time /t') do (set mytime=%%a%%b)

echo -------- [TEMPLATER JOB] -------- >> %log%
echo    Job completed on %mydate% @ %mytime% >> %log%
echo    Rendered job with ID %1 to %4 >> %log%
echo    Job details as JSON are found in file %7 >> %log%
@echo     >> %log%
